...
class MathSprite extends PositionComponent with TapCallbacks {
  MathSprite({
    required this.onTap,
    Vector2? position,
  }) : super(position: position ?? Vector2(160, 180), size: Vector2(120, 120));
  ...
  void setReaction(SpriteReaction reaction) {
    _reaction = reaction;
    if (reaction == SpriteReaction.celebrate) {
      _spawnSparkles();
    }
  }
  ...
  void _drawSprite(Canvas canvas, int stage) {
    ...
    final glowPaint = Paint()
      ..color = Color(0xFFFEF3C7).withAlpha(100);
    ...
    canvas.drawCircle(Offset(-10, -5), 6, eyePaint);
    canvas.drawCircle(Offset(10, -5), 6, eyePaint);

    final pupilPaint = Paint()..color = Color(0xFF1E293B);
    canvas.drawCircle(Offset(-9, -4), 3, pupilPaint);
    canvas.drawCircle(Offset(11, -4), 3, pupilPaint);
    ...
    switch (_reaction) {
      ...
      case SpriteReaction.excited:
        mouthPath.arcTo(Rect.fromCircle(center: Offset(0, 8), radius: 8), pi * 0.2, pi * 0.6, false);
        break;
      case SpriteReaction.encourage:
        mouthPath.moveTo(-6, 10);
        mouthPath.lineTo(6, 10);
        break;
      case SpriteReaction.thinking:
        mouthPath.moveTo(-5, 10);
        mouthPath.quadraticBezierTo(0, 5, 5, 10);
        break;
      default:
        mouthPath.moveTo(-4, 10);
        mouthPath.quadraticBezierTo(0, 6, 4, 10);
        break;
    }
    ...
    if (stage >= 1) {
      // Leaves
      final leafPaint = Paint()..color = Color(0xFF22C55E).withAlpha(180);
    ...
    if (stage >= 3) {
      ...
      final symbolPaint = Paint()..color = Color(0xFFFBBF24).withAlpha(150);
      _drawTextOnCanvas(canvas, 'π', Offset(-45, -35), symbolPaint, 14);
      _drawTextOnCanvas(canvas, '∑', Offset(40, -30), symbolPaint, 12);
      _drawTextOnCanvas(canvas, '∞', Offset(35, 25), symbolPaint, 12);
    }

    if (stage >= 4) {
      // Star crown
      final starPaint = Paint()..color = Color(0xFFFCD34D);
      canvas.drawPath(_starPath(size: 12, center: Offset(0, -42)), starPaint);
    }
  }
  ...
  void _drawTextOnCanvas(Canvas canvas, String text, Offset offset, Paint paint, double fontSize) {
    final builder = TextPainter(
      text: TextSpan(text: text, style: TextStyle(color: Color(0xFFFBBF24), fontSize: fontSize, fontStyle: FontStyle.italic)),
...
// Wrapper to embed Flame game in Flutter widget tree
import 'package:flame/game.dart';

class SpriteGameWidget extends StatelessWidget {
  final MathSprite sprite;

  const SpriteGameWidget({super.key, required this.sprite});

  @override
  Widget build(BuildContext context) {
    return GameWidget(
      game: _SpriteGame(sprite),
    );
  }
}
